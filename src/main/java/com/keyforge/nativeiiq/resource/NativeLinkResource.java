...
import com.keyforge.nativeiiq.service.NativeLinkExtractionService;
import com.keyforge.nativeiiq.wire.NativeLinkWire;
...
/**
 * Plugin REST resource exporting native IdentityIQ Link (account) data as JSON, read from the live
 * object model via {@link SailPointContext}. Mirrors the validated native resources: strictly
 ...
 *
 * <p>Full URL (from the IIQ base): {@code plugin/rest/keyForgeNativeIIQ/accounts?start=&limit=}.
 */
@Path("keyForgeNativeIIQ")
public class NativeLinkResource extends BasePluginResource {

    private static final Logger LOG = Logger.getLogger(NativeLinkResource.class.getName());
    ...
    public Response getAccounts(@QueryParam("start") @DefaultValue("0") int start,
        ...
        try {
            ...
            LOG.fine("KeyForgeNativeIIQ accounts: returning " + result.getCount() + " record(s)");
            // Return the Map so the provider serializes once to a JSON object (never a double-encoded String).
            return Response.ok(envelope).build();
        } catch (UnauthorizedAccessException e) {
            LOG.log(Level.WARNING, "KeyForgeNativeIIQ accounts: authorization denied at stage=" + stage, e);
            return error(Response.Status.FORBIDDEN, e);
        } catch (Throwable t) {
            LOG.log(Level.SEVERE, "KeyForgeNativeIIQ accounts: failed at stage=" + stage, t);
    ...
    private static Response error(Response.Status status, Throwable t) {
        return Response.status(status)
                .entity(NativeLinkWire.errorEnvelope(status.getStatusCode(), t))
