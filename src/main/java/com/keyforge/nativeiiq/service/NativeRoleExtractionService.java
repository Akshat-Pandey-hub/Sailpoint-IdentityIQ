...
import com.keyforge.nativeiiq.source.NativeRoleExtractor;

import sailpoint.api.SailPointContext;
import sailpoint.tools.GeneralException;

/**
 * Native (Java-API) Role ({@code Bundle}) extraction service — the transport-independent source
 * operation, mirroring the validated native services. Runs inside the IIQ runtime, reads
 * {@code sailpoint.object.Bundle} through the live {@link SailPointContext}, returns plain value
 * objects. Knows nothing about HTTP/REST/SCIM or PostgreSQL. STRICTLY READ-ONLY.
 */
public final class NativeRoleExtractionService {

    private final String sourceSystem;

    public NativeRoleExtractionService() {
        this("IdentityIQ");
    }

    public NativeRoleExtractionService(String sourceSystem) {
