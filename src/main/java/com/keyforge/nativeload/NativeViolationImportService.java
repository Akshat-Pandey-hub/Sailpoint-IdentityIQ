...
/**
 * Orchestrates the native PolicyViolation import: page the plugin endpoint, parse, upsert, then run the
 * confirmed-full-scan deletion sweep (empty-source safety guard). No HTTP, no JDBC — unit-testable.
 */
public final class NativeViolationImportService {
    ...
    private final NativeViolationPageSource source;
    private final NativeViolationParser parser = new NativeViolationParser();
    private final NativeViolationSink sink;
    private final int pageSize;
    private final boolean sweepDeletions;

    public NativeViolationImportService(NativeViolationPageSource source, NativeViolationSink sink,
    ...
    public Result importAll() throws SQLException {
        ...
        while (true) {
            ...
            List<NativeViolationRecord> page = parser.parse(json);
            if (page.isEmpty()) {
                break;
            }
            for (NativeViolationRecord rec : page) {
                extracted++;
                keep.add(NativeViolationRepository.canonicalViolationId(rec));
                try {
                    NativeViolationRepository.UpsertOutcome outcome = sink.upsert(rec);
                    if (outcome == NativeViolationRepository.UpsertOutcome.INSERTED) {
                        inserted++;
                    } else {
                        updated++;
                    }
