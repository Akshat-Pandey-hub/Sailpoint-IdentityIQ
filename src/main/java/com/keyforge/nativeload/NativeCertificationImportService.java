...
/**
 * Orchestrates the native Certification import (current-state): page → parse → upsert → confirmed-full-scan
 ...
 */
public final class NativeCertificationImportService {
    ...
    private final NativeCertificationPageSource source;
    private final NativeCertificationParser parser = new NativeCertificationParser();
    private final NativeCertificationSink sink;
    private final int pageSize;
    private final boolean sweepDeletions;

    public NativeCertificationImportService(NativeCertificationPageSource source, NativeCertificationSink sink,
    ...
    public Result importAll() throws SQLException {
        ...
        while (true) {
            ...
            List<NativeCertificationRecord> page = parser.parse(json);
            if (page.isEmpty()) {
                break;
            }
            for (NativeCertificationRecord rec : page) {
                extracted++;
                keep.add(NativeCertificationRepository.canonicalCertificationId(rec));
                try {
                    NativeCertificationRepository.UpsertOutcome outcome = sink.upsert(rec);
                    if (outcome == NativeCertificationRepository.UpsertOutcome.INSERTED) {
                        inserted++;
                    } else {
                        updated++;
                    }
        ...
        if (sourceCount >= 0 && extracted != sourceCount) {
            throw new NativeImportException("Incomplete Certification scan: sourceCount=" + sourceCount
