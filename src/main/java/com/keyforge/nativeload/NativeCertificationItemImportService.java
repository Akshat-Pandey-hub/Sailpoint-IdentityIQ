...
/**
 * Orchestrates the native CertificationItem import (current-state): page → parse → upsert → confirmed-full-
 * scan sweep. The {@code countObjects} total is verified against the extracted count; an incomplete scan
 * throws before sweeping so a partial pull never marks live rows deleted.
 */
public final class NativeCertificationItemImportService {
    ...
    private final NativeCertificationItemPageSource source;
    private final NativeCertificationItemParser parser = new NativeCertificationItemParser();
    private final NativeCertificationItemSink sink;
    private final int pageSize;
    private final boolean sweepDeletions;

    public NativeCertificationItemImportService(NativeCertificationItemPageSource source,
                                                NativeCertificationItemSink sink,
    ...
    public Result importAll() throws SQLException {
        ...
        while (true) {
            ...
            List<NativeCertificationItemRecord> page = parser.parse(json);
            if (page.isEmpty()) {
                break;
            }
            for (NativeCertificationItemRecord rec : page) {
                extracted++;
                keep.add(NativeCertificationItemRepository.canonicalItemId(rec));
                try {
                    NativeCertificationItemRepository.UpsertOutcome outcome = sink.upsert(rec);
                    if (outcome == NativeCertificationItemRepository.UpsertOutcome.INSERTED) {
                        inserted++;
                    } else {
                        updated++;
                    }
        ...
        if (sourceCount >= 0 && extracted != sourceCount) {
            throw new NativeImportException("Incomplete CertificationItem scan: sourceCount=" + sourceCount
