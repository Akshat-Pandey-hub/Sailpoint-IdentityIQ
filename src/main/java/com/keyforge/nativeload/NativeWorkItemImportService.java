...
/**
 * Orchestrates the native WorkItem import (current-state): page → parse → upsert → confirmed-full-scan
 * sweep. The authoritative {@code sourceCount} total is verified against the extracted count; an incomplete
 * scan throws before sweeping so a partial pull never marks live rows deleted.
 */
public final class NativeWorkItemImportService {
    ...
    private final NativeWorkItemPageSource source;
    private final NativeWorkItemParser parser = new NativeWorkItemParser();
    private final NativeWorkItemSink sink;
    private final int pageSize;
    private final boolean sweepDeletions;

    public NativeWorkItemImportService(NativeWorkItemPageSource source, NativeWorkItemSink sink,
    ...
    public Result importAll() throws SQLException {
        ...
        while (true) {
            ...
            List<NativeWorkItemRecord> page = parser.parse(json);
            if (page.isEmpty()) {
                break;
            }
            for (NativeWorkItemRecord rec : page) {
                extracted++;
                keep.add(NativeWorkItemRepository.canonicalWorkItemId(rec));
                try {
                    NativeWorkItemRepository.UpsertOutcome outcome = sink.upsert(rec);
                    if (outcome == NativeWorkItemRepository.UpsertOutcome.INSERTED) {
                        inserted++;
                    } else {
                        updated++;
                    }
                    ...
                    failed++;
                    if (failures.size() < 50) {
                        failures.add((rec.name != null ? rec.name : rec.sourceId) + ": "
                                + e.getClass().getName());
                    }
                }
            }
        ...
        if (sourceCount >= 0 && extracted != sourceCount) {
            throw new NativeImportException("Incomplete WorkItem scan: sourceCount=" + sourceCount
