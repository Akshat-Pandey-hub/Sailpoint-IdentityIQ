...
/**
 * Orchestrates the native ManagedAttribute import: pull the plugin endpoint page-by-page (via a
 * {@link NativeManagedAttributePageSource}), parse each page, and upsert every record (via a
 * {@link NativeManagedAttributeSink}). Pure orchestration — no HTTP and no JDBC — so it is unit-testable
 * with fakes. Mirrors {@code NativeIdentityImportService}, including the confirmed-full-scan deletion
 * sweep and its empty-source safety guard.
 */
public final class NativeManagedAttributeImportService {
    ...
    private final NativeManagedAttributePageSource source;
    private final NativeManagedAttributeParser parser = new NativeManagedAttributeParser();
    private final NativeManagedAttributeSink sink;
    private final int pageSize;
    private final boolean sweepDeletions;

    public NativeManagedAttributeImportService(NativeManagedAttributePageSource source,
                                               NativeManagedAttributeSink sink,
    ...
    public Result importAll() throws SQLException {
        ...
        while (true) {
            ...
            List<NativeManagedAttributeRecord> page = parser.parse(json);
            if (page.isEmpty()) {
                break;
            }
            for (NativeManagedAttributeRecord rec : page) {
                extracted++;
                keep.add(NativeManagedAttributeRepository.canonicalEntitlementId(rec));
                try {
                    NativeManagedAttributeRepository.UpsertOutcome outcome = sink.upsert(rec);
                    if (outcome == NativeManagedAttributeRepository.UpsertOutcome.INSERTED) {
                        inserted++;
                    } else {
                        updated++;
                    }
