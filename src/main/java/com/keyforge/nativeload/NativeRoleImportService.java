...
/**
 * Orchestrates the native Role (Bundle) import: pull the plugin endpoint page-by-page (via a
 * {@link NativeRolePageSource}), parse each page, upsert every record (via a {@link NativeRoleSink}).
 * Pure orchestration — no HTTP and no JDBC — unit-testable with fakes. Mirrors the validated native
 * imports: confirmed-full-scan deletion sweep with empty-source safety guard.
 */
public final class NativeRoleImportService {
    ...
    private final NativeRolePageSource source;
    private final NativeRoleParser parser = new NativeRoleParser();
    private final NativeRoleSink sink;
    private final int pageSize;
    private final boolean sweepDeletions;

    public NativeRoleImportService(NativeRolePageSource source, NativeRoleSink sink,
    ...
    public Result importAll() throws SQLException {
        ...
        while (true) {
            ...
            List<NativeRoleRecord> page = parser.parse(json);
            if (page.isEmpty()) {
                break;
            }
            for (NativeRoleRecord rec : page) {
                extracted++;
                keep.add(NativeRoleRepository.canonicalRoleId(rec));
                try {
                    NativeRoleRepository.UpsertOutcome outcome = sink.upsert(rec);
                    if (outcome == NativeRoleRepository.UpsertOutcome.INSERTED) {
                        inserted++;
                    } else {
                        updated++;
                    }
