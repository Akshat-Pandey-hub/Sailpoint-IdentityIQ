...
/**
 * Orchestrates the native Policy import: page the plugin endpoint, parse, upsert, then run the
 * confirmed-full-scan deletion sweep (empty-source safety guard). No HTTP, no JDBC — unit-testable.
 */
public final class NativePolicyImportService {
    ...
    private final NativePolicyPageSource source;
    private final NativePolicyParser parser = new NativePolicyParser();
    private final NativePolicySink sink;
    private final int pageSize;
    private final boolean sweepDeletions;

    public NativePolicyImportService(NativePolicyPageSource source, NativePolicySink sink,
    ...
    public Result importAll() throws SQLException {
        ...
        while (true) {
            ...
            List<NativePolicyRecord> page = parser.parse(json);
            if (page.isEmpty()) {
                break;
            }
            for (NativePolicyRecord rec : page) {
                extracted++;
                keep.add(NativePolicyRepository.canonicalPolicyId(rec));
                try {
                    NativePolicyRepository.UpsertOutcome outcome = sink.upsert(rec);
                    if (outcome == NativePolicyRepository.UpsertOutcome.INSERTED) {
                        inserted++;
                    } else {
                        updated++;
                    }
